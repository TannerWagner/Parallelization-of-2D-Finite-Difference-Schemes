...
def compute_fd(n, nt, k, f, fpp_num):
    ...
    f_vals = f(X, Y)
    f_vals_local = f_vals[start_halo * n: end_halo * n]
    
    # Task:
    # Compute the correct range of output values for this thread
    output = A*f_vals_local
    if k == 0:
...
def fcnpp(x, y):
    ...
    '''
    fcnppx = (-2./9.) * (x + 1)**(-5./3.) * (cos((x + 1)**(1./3.) + (y + 1)**(1./3.)) - sin((x + 1)**(1./3.) + (y + 1)**(1./3.))) + \
             (1./9.) * (x + 1)**(-4./3.) * (-cos((x + 1)**(1./3.) + (y + 1)**(1./3.)) - sin((x + 1)**(1./3.) + (y + 1)**(1./3.)))

    fcnppy = (-2./9.) * (y + 1)**(-5./3.) * (cos((x + 1)**(1./3.) + (y + 1)**(1./3.)) - sin((x + 1)**(1./3.) + (y + 1)**(1./3.))) + \
             (1./9.) * (y + 1)**(-4./3.) * (-cos((x + 1)**(1./3.) + (y + 1)**(1./3.)) - sin((x + 1)**(1./3.) + (y + 1)**(1./3.)))
...
for i,nt in enumerate(num_threads):
    ...
    for j,n in enumerate(NN):
        ...
        boundary_points_y1 = (Y == 1)
        fpp_numeric[boundary_points_y1] += (1/h**2) * fcn(X[boundary_points_y1], Y[boundary_points_y1] + h)

        boundary_points_x0 = (X == 0)
        fpp_numeric[boundary_points_x0] += (1/h**2) * fcn(X[boundary_points_x0] - h, Y[boundary_points_x0])

        boundary_points_x1 = (X == 1)
        fpp_numeric[boundary_points_x1] += (1/h**2) * fcn(X[boundary_points_x1] + h, Y[boundary_points_x1])
...
plot_filename = f"error{nt}.png"
pyplot.figure()
pyplot.loglog(NN, error[i, :])  # Plot error values
pyplot.loglog(NN.astype(float),(NN.astype(float)**-2),label='Quadratic Convergence',linestyle='--') 
pyplot.xlabel('Grid size (n)')
pyplot.ylabel('L2 Error')
pyplot.legend(fontsize=12)
pyplot.tick_params(axis='both', which='major', labelsize=10)
pyplot.title('Error vs Grid size for thread {}'.format(nt), fontsize=14)
pyplot.savefig(plot_filename, dpi=500, format='png', bbox_inches='tight', pad_inches=0.0)
pyplot.close()
